<?php

namespace Gateway;

use App\Database;
use Gateway\GatewayInterface;

/**
 * Handled alle PDO queries naar de database
 */
class MoveGateway implements GatewayInterface
{
    private \PDO $conn;

    public function __construct(Database $database)
    {
        $this->conn = $database->getConnection();
    }

    /**
     * Pakt alle Moves uit de database
     * @return array
     */
    public function getAll(): array
    {
        $sql = "SELECT * FROM move";

        $stmt = $this->conn->query($sql);

        $data = [];
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $data[] = $row;
        }

        return $data;
    }

    /**
     * Voegt een Move toe aan de Database
     * @param array $data
     * @return string
     * returned string met de opgeslagen id
     */
    public function create(array $data): string
    {
        $sql = "INSERT INTO move (
        name,
        type_id,
        category,
        power,
        accuracy) VALUES (:name,
        :type_id,
        :category,
        :power,
        :accuracy)";

        $stmt = $this->conn->prepare($sql);

        $stmt->bindValue(':name', $data["name"], \PDO::PARAM_STR);
        $stmt->bindValue(':type_id', $data["type_id"], \PDO::PARAM_INT);
        $stmt->bindValue(':category', $data["category"], \PDO::PARAM_STR);
        $stmt->bindValue(':power', $data["power"], \PDO::PARAM_INT);
        $stmt->bindValue(':accuracy', $data["accuracy"], \PDO::PARAM_INT);

        $stmt->execute();

        return $this->conn->lastInsertId();
    }

    /**
     * zoekt naar een Move met de meegegeven id
     * @param string $id
     * @return array|false
     */
    public function get(string $id): array|false
    {
        $sql = "SELECT * FROM move WHERE id = :id";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':id', $id, \PDO::PARAM_INT);

        $stmt->execute();

        $data = $stmt->fetch(\PDO::FETCH_ASSOC);

        return $data;
    }

    /**
     * zoekt naar een Move met de meegegeven name
     * @param string $name
     * @return array|false
     */
    public function getWithName(string $name): array|false
    {
        $sql = "SELECT * FROM move WHERE name = :name";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':name', $name, \PDO::PARAM_STR);

        $stmt->execute();

        $data = $stmt->fetch(\PDO::FETCH_ASSOC);

        return $data;
    }

    /**
     * Update informatie over één Move in de database
     * @param array $current
     * @param array $new
     * @return int
     * returned int met aantal rijen die gewijzigd zijn
     */
    public function update(array $current, array $new): int
    {
        $sql = "UPDATE move SET
        name = :name,
        type_id = :type_id,
        category = :category,
        power = :power,
        accuracy = :accuracy
        WHERE id = :id";

        $stmt = $this->conn->prepare($sql);

        $stmt->bindValue(":name", $new["name"] ?? $current["name"], \PDO::PARAM_STR);
        $stmt->bindValue(":type_id", $new["type_id"] ?? $current["type_id"], \PDO::PARAM_INT);
        $stmt->bindValue(":category", $new["category"] ?? $current["category"], \PDO::PARAM_STR);
        $stmt->bindValue(":power", $new["power"] ?? $current["power"], \PDO::PARAM_INT);
        $stmt->bindValue(":accuracy", $new["accuracy"] ?? $current["accuracy"], \PDO::PARAM_INT);

        $stmt->bindValue(":id", $current["id"], \PDO::PARAM_INT);

        $stmt->execute();

        return $stmt->rowCount();
    }

    /**
     * verwijderd een Move met de gegeven id
     * @param string $id
     * @return int
     * returned hoeveel rijen er verwijderd zijn
     */
    public function delete(string $id): int
    {
        if (is_numeric($id)) {
            $sql = "DELETE FROM move WHERE id = :id";

            $stmt = $this->conn->prepare($sql);

            $stmt->bindValue(":id", $id, \PDO::PARAM_INT);
        } else {
            $sql = "DELETE FROM move WHERE name = :id";

            $stmt = $this->conn->prepare($sql);

            $stmt->bindValue(":id", $id, \PDO::PARAM_STR);
        }

        $stmt->execute();

        return $stmt->rowCount();
    }
}
